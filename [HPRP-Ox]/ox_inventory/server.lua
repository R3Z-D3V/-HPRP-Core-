...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	return {
		...
		id = right.id,
		label = right.player and locale('otherplayer') or right.label,
...
RegisterNetEvent('ox_inventory:throwItem', function(slot, coords)
	local _source = source
	local inventory = Inventory(_source)

	if not inventory then return end
	local itemData = inventory.items[slot]
	if itemData then
		Inventory.RemoveItem(inventory.id, itemData.name, itemData.count, itemData.metadata or nil, slot)
		exports.ox_inventory:CustomDrop('Drop', {
    		{itemData.name, itemData.count, itemData.metadata or nil},
		}, vec3(coords.x, coords.y, coords.z + 0.25))
	end
end)

RegisterNetEvent('ox_inventory:renameSlot', function(slot, name)
	local _source = source
	local inventory = Inventory(_source)
	...
	local itemData = inventory.items[slot]
	if itemData then
		if not itemData.metadata then itemData.metadata = {} end
		itemData.metadata.label = name
		Inventory.SetMetadata(inventory.id, slot, itemData.metadata)
	end
end)
...
RegisterCommand('convertinventory', function(source, args)
	...
	if not convert then
		return warn('Invalid conversion argument. Valid options: esx, qb, esxproperty')
	end
