...
local function canAffordItem(inv, currency, price)
	...
	if currency == 'card' and price >= 0 and server.getAccountMoney(inv.id, 'bank') >= price then
		canAfford = true
	else
	...
	return canAfford or {
		type = 'error',
		description = locale('cannot_afford', ('%s%s'):format(((currency == 'money' or currency == 'card') and locale('$') or math.groupdigits(price)), ((currency == 'money' or currency == 'card') and math.groupdigits(price) or ' '..Items(currency).label)))
	}
end

local function removeCurrency(inv, currency, price)
	if currency == 'card' then
		server.removeAccountMoney(inv.id, 'bank', price)
	else
...
lib.callback.register('ox_inventory:buyItem', function(source, data)
	if data.toType == 'player' then
		...
		if fromData then
			...
			local currency = fromData.currency or (data.payment or 'money')
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				local message = locale('purchased_for', count, metadata?.label or fromItem.label, ((currency == 'money' or currency == 'card') and locale('$') or math.groupdigits(price)), ((currency == 'money' or currency == 'card') and math.groupdigits(price) or ' '..Items(currency).label))
