...
function Inventory.SetSlot(inv, item, count, metadata, slot)
	...
	if currentSlot and newCount < 1 then
		if not currentSlot.name:find('clothes_') then
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	local clothingSlots = GetResourceState('p_itemclothes') == 'started' and exports['p_itemclothes']:getClothingSlots() or nil
	...
	if toSlotType == 'number' then
		-- if clothingSlots and clothingSlots[toSlot] then
	...
	elseif toSlotType == 'table' then
		...
		for i = 1, #toSlot do
			local data = toSlot[i]
			-- if clothingSlots and clothingSlots[data.slot] then
			-- 	toSlot[i] = nil
			-- 	goto skip
			-- end
			added += data.count
