    ...
    if not interruptProgress(data) then
        CreateThread(function()
            exports['tgiann-lumihud']:Progress({
                name = data.label and data.label:lower() or 'progress_bar',
                duration = data.duration,
                label = data.label,
                useWhileDead = data.useWhileDead,
                canCancel = data.canCancel,
                controlDisables = {}, -- Wir handhaben die Steuerungsdeaktivierungen selbst
                -- Weitere notwendige Parameter hinzufügen
            }, function(cancelled)
                if cancelled then
                    progress = false
                else
