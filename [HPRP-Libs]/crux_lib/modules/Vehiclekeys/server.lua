...
local function DetectVehicleKeysSystem()
    if VehicleKeysType ~= 'auto-detect' then
        ...
        if resourceToCheck and GetResourceState(resourceToCheck) == 'started' then
            print(("^2[INFO]^7 Using specified VehicleKeys system: %s"):format(VehicleKeysType))
            return VehicleKeysType
        else
            print(("^3[WARNING]^7 Specified VehicleKeys system '%s' not found or not started. Falling back to auto-detect-detection."):format(VehicleKeysType))
        end
    end

    -- auto-detect-detection logic
    print("^5[INFO]^7 auto-detect-detecting VehicleKeys system...")
    for type, resource in pairs(VehicleKeysTypes) do
        if GetResourceState(resource) == 'started' then
            print(("^2[INFO]^7 auto-detect-detected VehicleKeys system: %s"):format(type))
            return type
        end
    end

    -- Default fallback
    print("^3[WARNING]^7 No VehicleKeys system detected, falling back to ox_lib VehicleKeys")
    return 'ox'
end
...
local filePath = ('modules/VehicleKeys/%s/server.lua'):format(VehicleKeysType)
...
if fileContent then
    ...
    if fn then
        ...
        if success then
            print(('^2[INFO]^7 Successfully loaded the VehicleKeys module: %s'):format(VehicleKeysType))
            ...
            print(('^1[ERROR]^7 Error executing the VehicleKeys module: %s - %s'):format(VehicleKeysType, result))
        end
    else
        print(('^1[ERROR]^7 Failed to load the VehicleKeys module: %s - %s'):format(VehicleKeysType, err))
    end
else
    print(('^1[ERROR]^7 VehicleKeys module file not found: %s'):format(filePath))
end
