...
local function DetectBillingSystem()
    if billingType == 'default' then
        print("^2[INFO]^7 Using default billing system.")
        return 'default'
    end
    ...
    if billingType ~= 'auto-detect' then
        ...
        if resourceToCheck and GetResourceState(resourceToCheck) == 'started' then
            print(("^2[INFO]^7 Using specified billing system: %s"):format(billingType))
            return billingType
        else
            print(("^3[WARNING]^7 Specified billing system '%s' not found or not started. Falling back to auto-detect-detection."):format(billingType))
        end
    end

    print("^5[INFO]^7 auto-detect-detecting billing system...")
    -- auto-detect-detection logic
    for type, resource in pairs(billingTypes) do
        if GetResourceState(resource) == 'started' then
            print(("^2[INFO]^7 auto-detect-detected billing system: %s"):format(type))
            return type
        end
    end

    print("^3[WARNING]^7 No billing system detected, falling back to default billing")
...
local filePath = ('modules/Billing/%s/server.lua'):format(folder)
...
if fileContent then
    ...
    if fn then
        ...
        if success then
            print(('^2[INFO]^7 Successfully loaded the Billing module: %s'):format(billingType))
            ...
            print(('^1[ERROR]^7 Error executing the Billing module: %s - %s'):format(billingType, result))
        end
    else
        print(('^1[ERROR]^7 Failed to load the Billing module: %s - %s'):format(billingType, err))
    end
else
    print(('^1[ERROR]^7 Billing module file not found: %s'):format(filePath))
end
